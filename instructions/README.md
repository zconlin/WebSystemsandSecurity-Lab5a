...
the more recent you can get, the better. As of right now writing this lab, Python 3.11 is the current version and is recommended.
...
You should now be able to find an executable in your path called `pip3.11`. To install requests, run this command:
```
...
You should now be able to find an executable on your system for `python3.11`. Open it up. In the top left of the window, you
should see `Python 3.11` (or some version close to that), and a couple of lines down, you should see the `>>> ` prompt. Type this in, note the capitalization:
```Python
import requests
```
...
> Note: On some Windows systems, you might need to use the command `py`, instead of `python` or `python3.11`. Just make sure the version matches the one you downloaded.
...
1. There are 4 different API's set up for passing off this lab they are found at the following URLs:
    * https://s1.byu-itc-210.net:1337
    * https://s2.byu-itc-210.net:1338
    * https://s3.byu-itc-210.net:1339
    * https://s4.byu-itc-210.net:1340
2. To test these API's you will need to get a valid cookie. You do this by accessing the endpoint `/api/v1/auth/google` for example, for s1 you would go to https://s1.byu-itc-210.net:1337/api/v1/auth/google and sign in with a valid Google account. 
...
- [ ] 50 Points - Total of twelve unit tests, all passing on the working API, and failing at the right time on faulty APIs.
- [ ] 20 Points - Each of the six `API` methods has decent documentation
- [ ] 10 Points - Code is backed up on GitHub
- [ ] 10 Points - `README.md` contains instructions sufficient for another developer to use your tests
- [ ] 5 Points - Proper shebang at top of every `.py` file
- [ ] 5 Points - Proper use of `if __name__ == '__main__':`
