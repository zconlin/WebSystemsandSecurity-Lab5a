...
the more recent you can get, the better. As of right now writing this lab, Python 3.10.6 is the current version and is recommended.
...
You should now be able to find an executable in your path called `pip3.10`. To install requests, run this command:
```
...
You should now be able to find an executable on your system for `python3.10`. Open it up. In the top left of the window, you
should see `Python 3.10.6` (or some version close to that), and a couple of lines down, you should see the `>>> ` prompt. Type this in, note the capitalization:
```Python
import requests
```
